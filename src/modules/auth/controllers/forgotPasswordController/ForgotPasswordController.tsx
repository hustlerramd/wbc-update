...
const ForgotPasswordController: React.FC<ForgotPasswordControllerProps> = (
  props: ForgotPasswordControllerProps
) => {
  const { onClose } = props;
  const [forgotPasswordData, setForgotPasswordData] =
    useState<ForgotPasswordDataType>({ email: "" });
  const { state, isLoading, error, fieldErrors, execute, clearFieldError } =
    WBC_HOOKS.useActionState<
      ForgotPasswordDataType,
      ForgotPasswordResponseType
    >({ email: "" }, null);
  ...
  const handleOnchange = (name: string, value: string) =>
    ViewHelper.handleChangeHelper<ForgotPasswordDataType>(
      name,
      value,
      clearFieldError,
